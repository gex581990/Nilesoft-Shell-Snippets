...
menu(type="taskbar" vis=key.shift() or key.lbutton() pos=0 title=app.name image=\uE249)
{
	...
	item(title="add admin" tip='Add "@app.exe" to the list of applications that always run as administrator. Hold SHIFT to add for all users (requires admin rights).'
		image=\uE142
		vis=if(reg.exists(flags_m_reg, '@app.exe') or reg.exists(flags_u_reg, '@app.exe'), 'disable')
		admin cmd-line='/c reg.exe add "@if(keys.shift(), flags_m_reg, flags_u_reg)" /v "@app.exe"/t REG_SZ /d "RUNASADMIN" /f' window=cmd.hidden)
	...
	item(title="docs" tip='If the site is not available, please visit the archive.'
		...
		// 	cmd-ps=`$tcpClient = New-Object Net.Sockets.TcpClient; try { $tcpClient.Connect('nilesoft.org', 443); Start-Process "https://nilesoft.org/docs" } catch { Start-Process 'https://web.archive.org/web/@"*"/https://nilesoft.org' } finally { $tcpClient.Close() }`
		cmd-ps=`try {
		...
		window='hidden')
	$svg_deepwiki='<svg width="100" height="100" viewBox="0 0 128 128"><path fill="#40C0E7" d="M65.71 15.31h-43.6c-1.25 0-2.24 1-2.24 2.24v100.96c0 1.24 1 2.24 2.24 2.24H65.7c30.15 0 50.43-21.19 50.43-52.73 0-31.53-20.27-52.71-50.42-52.71m-1.29 80.8c-.28 0-.54-.07-.79-.16-.06.01-.11.03-.17.03-.08.01-.14.04-.22.04h-14.7c-1.2 0-2.16-.97-2.16-2.16V42.19c0-1.19.96-2.16 2.16-2.16h14.7c.08 0 .14.03.22.04.05 0 .11.02.17.03.25-.09.51-.16.79-.16.43 0 .86.04 1.29.06.75.03 1.5.09 2.24.18 13.11 1.63 21.69 12.39 21.69 27.84s-8.59 26.21-21.69 27.84c-.74.09-1.49.15-2.24.18-.43.03-.86.07-1.29.07" /></svg>'
	item(title="deepwiki" tip='Community driven documentation.'
		image=svg_deepwiki
		cmd='https://deepwiki.com/moudey/Shell')
